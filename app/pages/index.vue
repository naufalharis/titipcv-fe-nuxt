<template>
  <div>
    <!-- Maintenance Mode Full Page Redirect -->
    <div v-if="isMaintenanceMode" class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
      <div class="max-w-md w-full text-center">
        <!-- Icon -->
        <div class="mb-8">
          <div class="inline-flex p-4 bg-amber-100 dark:bg-amber-900/30 rounded-full mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-amber-600 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Divider -->
        <div class="w-20 h-1 bg-amber-400 mx-auto mb-6 rounded-full"></div>
    ...
    <!-- Normal Home Content (Only shown when maintenance is false) -->
    <template v-else>
      <!-- Page Header -->
...
const checkMaintenanceStatus = async () => {
  try {
    ...
    const newMaintenanceStatus = settings?.maintenanceMode || false
    
    // If maintenance mode status changed from true to false
    if (isMaintenanceMode.value === true && newMaintenanceStatus === false) {
      window.location.reload() // Reload to show normal content
    }
    
    isMaintenanceMode.value = newMaintenanceStatus
...
const fetchSiteSettings = async (): Promise<void> => {
  ...
  try {
    ...
    // TAMPILKAN POPUP OTOMATIS SETELAH DATA LOAD (hanya jika maintenance false)
