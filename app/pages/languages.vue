<template>
  <div class="animate-fade-in">
    <div class="mb-8 flex justify-between items-center">
      <div>
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Languages Management</h1>
        <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Manage all languages in your application.</p>
      </div>
      
      <!-- Add Language Button - only show in active view -->
      <button 
        v-if="!showDeleted"
    ...
    <DataTable 
      ...
      :title="showDeleted ? 'Deleted Languages' : 'Active Languages'" 
      :columns="columns" 
      :rows="languages"
      :loading="loading"
    >
      <!-- Customizing the Language cell -->
      <template #cell-language="{ row }">
        <div class="flex items-center gap-3">
          <div class="flex-shrink-0 w-10 h-10 bg-primary-50 dark:bg-primary-900/20 rounded-lg flex items-center justify-center text-lg font-semibold text-primary-600 dark:text-primary-400 uppercase">
            {{ (row as Language).code }}
          </div>
          <div>
            <div class="text-sm font-medium text-slate-900 dark:text-white">{{ (row as Language).name }}</div>
            <div class="text-xs text-slate-500 dark:text-slate-400">ID: {{ (row as Language).id.slice(0, 8) }}...</div>
          </div>
        </div>
      </template>
      ...
      <template #cell-createdAt="{ row }">
        <span class="text-sm text-slate-600 dark:text-slate-400">
          {{ formatDate((row as Language).createdAt) }}
        </span>
      </template>
      ...
      <template #cell-updatedAt="{ row }">
        <span class="text-sm text-slate-600 dark:text-slate-400">
          {{ formatDate((row as Language).updatedAt) }}
        </span>
      </template>
      ...
      <template #cell-deletedAt="{ row }">
        <span v-if="showDeleted" class="text-sm text-slate-600 dark:text-slate-400">
          {{ formatDate((row as Language).deletedAt) }}
        </span>
      ...
      <template #cell-status="{ row }">
        <span 
          ...
          :class="{
            'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300': !(row as Language).deletedAt,
            'bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-300': (row as Language).deletedAt
          }"
        >
          <span class="w-1.5 h-1.5 rounded-full mr-1.5" :class="{
            'bg-emerald-500': !(row as Language).deletedAt,
            'bg-slate-500': (row as Language).deletedAt
          }"></span>
          {{ (row as Language).deletedAt ? 'Deleted' : 'Active' }}
        </span>
      </template>
      ...
      <template #cell-actions="{ row }">
        <div class="flex items-center gap-2">
          <!-- Edit button - always show for non-deleted languages in active view -->
          <button 
            v-if="!showDeleted"
            @click="openEditModal(row as Language)"
          ...
          <!-- Restore button - show in deleted view -->
          <button 
            v-if="showDeleted"
            @click="handleRestore(row as Language)"
      ...
      <template #empty>
        <div class="text-center py-8 text-slate-500 dark:text-slate-400">
          {{ showDeleted ? 'No deleted languages found' : 'No languages found' }}
        </div>
      </template>
    </DataTable>
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="e.g., EN, ID, JP"
                    maxlength="10"
                    required
                  />
                  <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Standard language code (e.g., EN for English, ID for Indonesian)
                  </p>
                </div>
      ...
      <Transition name="modal-fade">
        <div v-if="showDeleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 backdrop-blur-sm" @click.self="showDeleteModal = false">
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md animate-modal-slide-up">
            <div class="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
              <h3 class="text-lg font-semibold text-slate-900 dark:text-white">
                {{ showDeleted ? 'Permanently Delete Language' : 'Delete Language' }}
              </h3>
            ...
            <div class="p-6">
              <p class="text-slate-600 dark:text-slate-400">
                ...
                language "{{ selectedLanguage?.name }}" ({{ selectedLanguage?.code }})?
                <span v-if="!showDeleted" class="block mt-2 text-sm text-amber-600 dark:text-amber-400">
                  Note: This will soft delete the language. You can restore it later from the deleted languages tab.
                </span>
                <span v-else class="block mt-2 text-sm text-red-600 dark:text-red-400">
                  Warning: This action cannot be undone. The language will be permanently removed from the database.
                </span>
              </p>
            </div>
...
const fetchLanguages = async () => {
  ...
  try {
    ...
    if (showDeleted.value) {
      // Fetch deleted languages
      response = await getDeletedLanguages(searchQuery.value || undefined)
    } else {
      if (searchQuery.value) {
        // Search languages
        response = await searchLanguages(searchQuery.value)
      } else {
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && selectedLanguage.value) {
      await updateLanguage(selectedLanguage.value.id, {
    ...
    error.value = err.message || `Failed to ${isEdit.value ? 'update' : 'create'} language`
...
const handleDelete = async () => {
  ...
  try {
    if (showDeleted.value) {
      // Permanent delete
      await permanentDeleteLanguage(selectedLanguage.value.id)
    } else {
      // Soft delete
      await deleteLanguage(selectedLanguage.value.id)
    }
    ...
    error.value = err.message || 'Failed to delete language'
...
// Restore handler
const handleRestore = async (language: Language) => {
  if (!confirm(`Are you sure you want to restore "${language.name}"?`)) {
    return
  }
  ...
  try {
    await restoreLanguage(language.id)
    await fetchLanguages()
  } catch (err: any) {
    error.value = err.message || 'Failed to restore language'
